...
const getOptimalItemForBin = (items, bin) => {
  ...
  items.map(({ item, index, spin }) => {
    if (bin.type === 'h' && item.l >= optimalItem.l) {
      if (item.l === optimalItem.l && item.h > optimalItem.h) {
        optimalItem = item;
        optimalItemIndex = index;
        optimalItemSpin = spin;  
      }
      else if (item.l > optimalItem.l) {
        optimalItem = item;
        optimalItemIndex = index;
        optimalItemSpin = spin;
      }
    ...
    <Fragment>
      ...
      {(subBins || []).map(subBin => {
        ...
        return (
          <mesh
            opacity={0.5}
            position={[subBin.cornerCoord.x + subBin.l / 2, subBin.cornerCoord.y + subBin.b / 2, subBin.cornerCoord.z + subBin.h / 2]}
            color='#CECECE'
          >         
            <boxBufferGeometry
              attach='geometry'
